...
export class Ingester {
  ...
  private async indexPost(uri: string, cid: string, authorDid: string, record: Record<string, unknown>) {
    ...
    if (replyParent) {
      await this.notifyPostAuthor(replyParent, authorDid, 'reply')
    }
  }
  ...
  private async indexLike(uri: string, authorDid: string, record: Record<string, unknown>) {
    ...
    await this.notifyPostAuthor(subject.uri, authorDid, 'like')
  }
  ...
  private async indexRepost(uri: string, authorDid: string, record: Record<string, unknown>) {
    ...
    // Send push notification to post author
    await this.notifyPostAuthor(subject.uri, authorDid, 'repost')
  }
  ...
  private async indexFollow(uri: string, authorDid: string, record: Record<string, unknown>) {
    ...
    await this.notifyFollow(subject, authorDid)
  }

  /**
   * Send push notification to a post's author
   */
  private async notifyPostAuthor(postUri: string, actorDid: string, action: 'like' | 'repost' | 'reply') {
    // Don't notify yourself
    const postAuthorDid = postUri.split('/')[2]
    if (postAuthorDid === actorDid) return

    // Get actor's profile for display name
    const actor = this.db.prepare('SELECT handle, display_name FROM profiles WHERE did = ?').get(actorDid) as { handle: string; display_name: string | null } | undefined
    const actorName = actor?.display_name || actor?.handle || actorDid.slice(0, 20)

    const messages: Record<string, { title: string; body: string }> = {
      like: { title: 'New Like', body: `${actorName} liked your post` },
      repost: { title: 'New Repost', body: `${actorName} reposted your post` },
    ...
    await sendPushToUser(this.db, postAuthorDid, payload)
  }

  /**
   * Send push notification for new follow
   */
  private async notifyFollow(followedDid: string, followerDid: string) {
    // Get follower's profile
    const follower = this.db.prepare('SELECT handle, display_name FROM profiles WHERE did = ?').get(followerDid) as { handle: string; display_name: string | null } | undefined
    const followerName = follower?.display_name || follower?.handle || followerDid.slice(0, 20)
    ...
    await sendPushToUser(this.db, followedDid, payload)
  }
