...
import { saveSubscription, removeSubscription, getVapidPublicKey, isPushEnabled } from './push.js'
...
app.post('/api/push/subscribe', (req, res) => {
  try {
    ...
    if (!userDid || !subscription?.endpoint || !subscription?.keys?.p256dh || !subscription?.keys?.auth) {
    ...
    const success = saveSubscription(db, userDid, subscription, userAgent)
...
app.delete('/api/push/unsubscribe', (req, res) => {
  try {
    ...
    const success = removeSubscription(db, endpoint)
    res.json({ success })
  } catch (err) {
